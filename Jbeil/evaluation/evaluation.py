...
import numpy as np
import torch
from sklearn.metrics import average_precision_score, roc_auc_score, confusion_matrix, recall_score, precision_score, \
    roc_curve
...
def eval_edge_prediction(model, negative_edge_sampler, data, n_neighbors, batch_size=200):
    ...
    return np.mean(val_ap), np.mean(val_auc), np.mean(val_recall), np.mean(val_precision), np.mean(val_fp), np.mean(
        val_fn), np.mean(val_tp), np.mean(val_tn)
